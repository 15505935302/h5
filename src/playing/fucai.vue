<template>
  <div id="playing_28" :sutest="this.$store.state.gameType.game_type" v-if="loading">
    <template v-if="this.$store.state.wanfaName==4">
      <PlayHead
        :gameList="gameList[this.$store.state.wanfaName].lists[this.$store.state.wanfachild]"
      ></PlayHead>
    </template>
    <template v-else>
    ...
    <template v-else>
      <Qihao></Qihao>
      <pk1053
        v-if="this.$store.state.wanfaName==1 || this.$store.state.wanfaName==2"
        :gameList="gameList[this.$store.state.wanfaName]"
      ></pk1053>
      <pk1053
        v-else-if="this.$store.state.wanfaName==4"
        :gameList="gameList[this.$store.state.wanfaName].lists[this.$store.state.wanfachild]"
        :firstMenu="gameList[this.$store.state.wanfaName]"
      ></pk1053>
      <PK10Size v-else :gameList="gameList[this.$store.state.wanfaName]"></PK10Size>
...
export default {
  ...
  methods: {
    ...
    betRequest() {
      this.$api
        .wanfalist({ baseWanfaId: this.$store.state.gameType.id })
        .then(res => {
