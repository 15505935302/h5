    ...
    <div class="main child-view">
      ...
      <div id="leftTabBox">
        <div class="hd">
          <ul @click="statusType($event)">
            <li data-id="0" :class="{on:status==0}">全部</li>
            <li data-id="1" :class="{on:status==1}">待开奖</li>
            <li data-id="2" :class="{on:status==2}">已中奖</li>
            <li data-id="3" :class="{on:status==3}">未中奖</li>
          </ul>
        </div>
      ...
      <div v-show="newdata">
        ...
        <div class="filter">
          ...
          <div class="tp bot">
            <span data-id="1" :class="{active:choose==0}"  @click="choosed($event,0)">全部</span>
            <span v-for="(item, index) in wanfalist"  @click="choosed($event,item.id)" :key="index" data-id="index+2" :class="{active:choose==item.id}">
              {{ item.name }}
            </span>
          </div>
          <button @click="subbetlist">筛选</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    subbetlist(){
      ...
      this.newdata = false
      this.$api.subbetlist(obj).then(res=>{
